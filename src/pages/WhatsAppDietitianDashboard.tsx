...
const WhatsAppDietitianDashboard: React.FC = () => {
  ...
  // Mock data for dietitian chats
  const mockChats: DietitianChat[] = [
    {
      id: '1',
  ...
  const calculateStats = (chatData: DietitianChat[]) => {
    ...
    const avgResponseTime = 1.8; // Mock average response time in minutes
    const satisfactionRate = 85; // Mock satisfaction rate
    const escalationRate = 20; // Mock escalation rate
    const aiAccuracy = 91.5; // Mock AI accuracy rate
  ...
  const handleAutomationControl = (action: string) => {
    console.log(`WhatsApp Dietitian Assistant Automation: ${action}`);
  ...
  const loadPendingRequests = async () => {
    try {
      ...
      const { data: requests, error } = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('workflow_name', 'WhatsApp Dietitian Assistant')
        .eq('status', 'pending')
  ...
  const handleWorkflowRequest = async (action: 'start' | 'stop' | 'modify') => {
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          user_id: user.id,
          workflow_name: 'WhatsApp Dietitian Assistant',
          request_type: action,
          status: 'pending',
          request_details: `User requested to ${action} the WhatsApp Dietitian Assistant workflow`,
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-green-800 text-sm">
                  🎉 Showing real-time WhatsApp Dietitian Assistant metrics from your workflow executions
                </p>
              </div>
            </div>
          </div>
...
export default WhatsAppDietitianDashboard; 
